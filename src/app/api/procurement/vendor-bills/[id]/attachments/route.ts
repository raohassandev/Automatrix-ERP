...
export async function GET(_req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const bill = await prisma.vendorBill.findUnique({ where: { id }, select: { id: true } });
  if (!bill) {
    return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  }

  const rows = await prisma.attachment.findMany({
    where: { type: "vendor_bill", recordId: id },
...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (isProcurementAttachmentLocked("vendor_bill", bill.status)) {
    const error = procurementAttachmentLockMessage("Vendor Bill", bill.status);
    await logAudit({
      action: "BLOCK_VENDOR_BILL_ATTACHMENT_LOCKED",
      entity: "VendorBill",
      entityId: id,
      reason: error,
  ...
  await logAudit({
    action: "VENDOR_BILL_ATTACHMENT_ADD",
    entity: "VendorBill",
    entityId: id,
