...
export async function GET(_req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const po = await prisma.purchaseOrder.findUnique({ where: { id }, select: { id: true } });
  if (!po) {
    return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  }

  const rows = await prisma.attachment.findMany({
    where: { type: "purchase_order", recordId: id },
...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (isProcurementAttachmentLocked("purchase_order", po.status)) {
    const error = procurementAttachmentLockMessage("Purchase Order", po.status);
    await logAudit({
      action: "BLOCK_PURCHASE_ORDER_ATTACHMENT_LOCKED",
