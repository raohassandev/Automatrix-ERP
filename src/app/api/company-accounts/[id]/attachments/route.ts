...
import { z } from "zod";
import { isCompanyAccountAttachmentLocked, validateAttachmentFormat } from "@/lib/document-attachment-policy";
...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const account = await prisma.companyAccount.findUnique({ where: { id }, select: { id: true, isActive: true } });
  if (!account) {
    return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  }
  if (isCompanyAccountAttachmentLocked(account.isActive)) {
    const error = "Attachments are locked because company account is inactive.";
    await logAudit({
