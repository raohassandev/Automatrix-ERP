...
// ============================================================================
// Build Date:
// Generated: Tue Jan 27 17:20:31 PKT 2026
//
...
function getPendingApprovalsEnhanced(filters = {}) {
  try {
        ...
        if (status === STATUS.PENDING || status.startsWith('Pending')) {
          ...
          if (canApproveAmount(userEmail, 'EXPENSE', amount)) {
            approvals.push({
              ...
              hasReceipt: !!(
                data[i][EXPENSES_COLS.RECEIPT_FILE_ID] ||
                data[i][EXPENSES_COLS.RECEIPT_URL]
              ),
              daysWaiting: getDaysBetween(
                new Date(data[i][EXPENSES_COLS.DATE]),
                new Date(),
              ),
        ...
        if (status === STATUS.PENDING || status.startsWith('Pending')) {
          if (canApproveAmount(userEmail, 'INCOME', amount)) {
            approvals.push({
              ...
              daysWaiting: getDaysBetween(
                new Date(data[i][INCOME_COLS.DATE]),
                new Date(),
              ),
              formattedAmount: formatCurrency(amount),
              formattedDate: formatDateForDisplay(
    ...
    if (filters.minAmount) {
      filteredApprovals = filteredApprovals.filter(
        (a) => a.amount >= filters.minAmount,
      );
    }

    if (filters.maxAmount) {
      filteredApprovals = filteredApprovals.filter(
        (a) => a.amount <= filters.maxAmount,
      );
    }
    ...
    return {
      ...
      summary: {
        ...
        expenseCount: filteredApprovals.filter((a) => a.type === 'EXPENSE')
          .length,
        incomeCount: filteredApprovals.filter((a) => a.type === 'INCOME')
          .length,
      },
    };
  } catch (error) {
  ...
  try {
    ...
    return preventDoubleApproval(type, rowNum, () => {
      ...
      if (action === 'APPROVE') {
        ...
        const finalStatus = isPartialApproval
          ? STATUS.PARTIALLY_APPROVED
          : STATUS.APPROVED;
        ...
        if (type === 'EXPENSE') {
          ...
          if (!isPartialApproval) {
            updateWalletBalance(
              submittedBy,
              -amount,
              `Expense ${rowNum} approved`,
            );
          } else {
            updateWalletBalance(
              submittedBy,
              -approvedAmount,
              `Expense ${rowNum} partially approved`,
            );
          }
        } else {
          sheet.getRange(rowNum, INCOME_COLS.STATUS + 1).setValue(finalStatus);
          sheet
            .getRange(rowNum, INCOME_COLS.APPROVED_BY + 1)
            .setValue(userEmail);
          sheet
            .getRange(rowNum, INCOME_COLS.APPROVED_DATE + 1)
            .setValue(new Date());
        }
        ...
        if (type === 'EXPENSE') {
          ...
          sheet
            .getRange(rowNum, INCOME_COLS.STATUS + 1)
            .setValue(STATUS.REJECTED);
          sheet
            .getRange(rowNum, INCOME_COLS.APPROVED_BY + 1)
            .setValue(userEmail);
...
function checkMandatoryFields(rowData, type) {
  ...
  if (type === 'EXPENSE') {
    ...
    if (amount >= VALIDATION_RULES.RECEIPT_MANDATORY_THRESHOLD) {
      if (
        !rowData[EXPENSES_COLS.RECEIPT_FILE_ID] &&
        !rowData[EXPENSES_COLS.RECEIPT_URL]
      ) {
        missing.push(
          `Receipt is mandatory for amounts ≥ ${formatCurrency(VALIDATION_RULES.RECEIPT_MANDATORY_THRESHOLD)}`,
        );
      }
    }
  }
...
function updateWalletBalance(employeeEmail, amount, reference) {
  try {
    ...
    empSheet
      .getRange(profile.rowIndex, EMPLOYEES_COLS.WALLET_BALANCE + 1)
      .setValue(newBalance);
...
function bulkApprove(items) {
  try {
    ...
    items.forEach((item) => {
      try {
        const result = processApprovalEnhanced(
          item.rowNum,
          item.type,
          'APPROVE',
          item.data || {},
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (isDateInRange(submitDate, dates.start, dates.end)) {
          if (
            ...
            if (approvedBy) {
              stats.approvalsByUser[approvedBy] =
                (stats.approvalsByUser[approvedBy] || 0) + 1;
            }

            if (approvedDate) {
              const timeDiff = getDaysBetween(
                submitDate,
...
function attachFileToExpense(expenseRowNum, fileId, fileUrl) {
  try {
    ...
    sheet
      .getRange(expenseRowNum, EXPENSES_COLS.RECEIPT_FILE_ID + 1)
      .setValue(fileId);
    sheet
      .getRange(expenseRowNum, EXPENSES_COLS.RECEIPT_URL + 1)
      .setValue(fileUrl);
...
function attachFileToIncome(incomeRowNum, fileId, fileUrl) {
  try {
    ...
    // Update receipt fields
    sheet
      .getRange(incomeRowNum, INCOME_COLS.RECEIPT_FILE_ID + 1)
      .setValue(fileId);
...
function getSummary() {
  try {
    ...
    const projects = pRaw.map((r) => ({
      name: r[PROJECTS_COLS.NAME],
      income: r[PROJECTS_COLS.INVOICED_AMOUNT] || 0,
    }));
    ...
    return projects.map((p) => {
      const projectExpenses = expenses.filter(
        (e) => e.project === p.name && e.status === 'Approved',
      );
      const totalExpense = projectExpenses.reduce(
...
function calculateKPIs(dates, userEmail, role) {
  ...
  if (projectSheet) {
    ...
    for (let i = 1; i < projectData.length; i++) {
      const pendingRecovery =
        parseFloat(projectData[i][PROJECTS_COLS.PENDING_RECOVERY]) || 0;
  ...
  return {
    ...
    profitMargin:
      kpis.totalIncome > 0 ? (kpis.netProfit / kpis.totalIncome) * 100 : 0,
  };
}
...
function calculateTrends(dates, userEmail, role) {
  ...
  const trends = {
    ...
    recovery: formatTrend(
      currentKPIs.pendingRecovery,
      previousKPIs.pendingRecovery,
    ),
  };
...
function getProjectNames() {
  ...
  return rows
    .slice(1)
    .map((row) => row[PROJECTS_COLS.NAME])
    .filter((name) => !!name);
}
...
function getDateRangeLabel(period, dates) {
  switch (period) {
    ...
    case 'CUSTOM':
      return (
        formatDateForDisplay(dates.start) +
        ' - ' +
        formatDateForDisplay(dates.end)
      );
    default:
      return period;
  }
...
function getPendingRecoveryDetails(userEmail, role) {
  ...
  for (let i = 1; i < data.length; i++) {
    const pendingAmount =
      parseFloat(data[i][PROJECTS_COLS.PENDING_RECOVERY]) || 0;
...
function getEmployeesEnhanced(filters = {}) {
  try {
    ...
    // Check permission
    if (
      !hasPermission(userEmail, 'employees.view_all') &&
      !hasPermission(userEmail, 'employees.view_team')
    ) {
      return {
        success: false,
        error: 'Access denied',
      ...
      if (data[i][WALLET_COLS.EMPLOYEE] === email) {
        transactions.push({
          ...
          formattedAmount: formatCurrency(
            parseFloat(data[i][WALLET_COLS.AMOUNT]) || 0,
          ),
          formattedBalance: formatCurrency(
            parseFloat(data[i][WALLET_COLS.BALANCE]) || 0,
          ),
          formattedDate: formatDateForDisplay(
            new Date(data[i][WALLET_COLS.DATE]),
          ),
        });
      }
    }
...
function getExpensesEnhanced(filters = {}) {
  try {
    ...
    return {
      ...
      totalPages: filters.pageSize
        ? Math.ceil(expenses.length / filters.pageSize)
        : 1,
    };
  } catch (error) {
...
function getExpenseById(rowNum) {
  try {
    ...
    const canApprove =
      canApproveAmount(userEmail, 'EXPENSE', expense.amount) &&
  ...
  try {
    ...
    return withLock(`update_expense_${rowNum}`, () => {
      ...
      if (updates.description) {
        updateMap[EXPENSES_COLS.DESCRIPTION] = sanitizeInput(
          updates.description,
        );
      }
      if (updates.category) {
      ...
      if (updates.amount) {
        ...
        updateMap[EXPENSES_COLS.APPROVAL_LEVEL] = approvalLevel
          ? approvalLevel.level
          : 'L1';
      }
...
function getExpenseCategories() {
  try {
    const categories = getUniqueValues(
      SHEET_NAMES.EXPENSES,
      EXPENSES_COLS.CATEGORY,
    );

    // Count usage for each category
    const categoriesWithCount = categories
      .map((cat) => {
        const count = countRowsByValue(
          SHEET_NAMES.EXPENSES,
          EXPENSES_COLS.CATEGORY,
          cat,
        );
...
function getExpenseSummaryByCategory(dateRange = 'THIS_MONTH') {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (
        isDateInRange(expenseDate, dates.start, dates.end) &&
        status === STATUS.APPROVED
      ) {
        if (!summary[category]) {
...
function getIncomeEnhanced(filters = {}) {
  try {
    ...
    // Check permission
    if (
      !hasPermission(userEmail, 'income.view_all') &&
      !hasPermission(userEmail, 'income.view_own')
    ) {
      return {
        success: false,
        error: 'Access denied',
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (
        dateRange &&
        !isDateInRange(new Date(income.date), dateRange.start, dateRange.end)
      ) {
        continue;
      }
...
function getIncomeSummaryByCategory(dateRange = 'THIS_MONTH') {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      // Filter by date and status
      if (
        isDateInRange(incomeDate, dates.start, dates.end) &&
        status === STATUS.APPROVED
      ) {
        if (!summary[category]) {
          summary[category] = { total: 0, count: 0 };
...
function updateIncome(rowNum, updates) {
  try {
    ...
    if (
      !isAdmin &&
      (income.addedBy !== userEmail || income.status !== STATUS.PENDING)
    ) {
      return {
        success: false,
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      const item = {
        ...
        avgUsage30Days:
          parseFloat(data[i][INVENTORY_COLS.AVG_USAGE_30_DAYS]) || 0,
...
function recordInventoryTransaction(transaction) {
  try {
    ...
    return withLock(`inventory_${transaction.itemName}`, () => {
      ...
      const itemData = invSheet
        .getRange(itemRow, 1, 1, invSheet.getLastColumn())
        .getValues()[0];
      ...
      if (quantityChange < 0 && newQty < 0) {
        throw new Error(
          `Insufficient stock. Available: ${currentQty}, Required: ${Math.abs(quantityChange)}`,
        );
      }
      ...
      const unitCost =
        ...
        parseFloat(itemData[INVENTORY_COLS.UNIT_COST]) ||
        0;
      ...
      invSheet
        .getRange(itemRow, INVENTORY_COLS.TOTAL_VALUE + 1)
        .setValue(newTotalValue);

      // Update available quantity (quantity - reserved)
      const reservedQty =
        parseFloat(itemData[INVENTORY_COLS.RESERVED_QTY]) || 0;
      const availableQty = newQty - reservedQty;
      invSheet
        .getRange(itemRow, INVENTORY_COLS.AVAILABLE_QTY + 1)
        .setValue(availableQty);

      // Update last updated
      invSheet
        .getRange(itemRow, INVENTORY_COLS.LAST_UPDATED + 1)
        .setValue(new Date());

      // Update last purchase date if this is a purchase
      if (transaction.type === STATUS.PURCHASE) {
        invSheet
          .getRange(itemRow, INVENTORY_COLS.LAST_PURCHASE_DATE + 1)
          .setValue(new Date());
      }

      // Log audit
      logInventoryLedgerEntry(
        transaction.itemName,
        transaction.type,
        quantityChange,
        transaction.reference,
    ...
    for (let i = data.length - 1; i >= 1 && entries.length < limit; i--) {
      if (data[i][INVENTORY_LEDGER_COLS.ITEM] === itemName) {
        entries.push({
          ...
          runningBalance:
            parseFloat(data[i][INVENTORY_LEDGER_COLS.RUNNING_BALANCE]) || 0,
          formattedDate: formatDateForDisplay(
            new Date(data[i][INVENTORY_LEDGER_COLS.DATE]),
          ),
          formattedTotal: formatCurrency(
            parseFloat(data[i][INVENTORY_LEDGER_COLS.TOTAL]) || 0,
          ),
        });
      }
    }
...
function updateInventoryItem(itemName, updates) {
  try {
    ...
    return withLock(`inventory_${itemName}`, () => {
      ...
      // Update last updated
      sheet
        .getRange(rowNum, INVENTORY_COLS.LAST_UPDATED + 1)
        .setValue(new Date());
      ...
      if (minStock > 0 && quantity <= minStock) {
        reorderItems.push({
          ...
          estimatedCost:
            ...
            (parseFloat(data[i][INVENTORY_COLS.UNIT_COST]) || 0),
...
function reserveStock(itemName, quantity, projectId) {
  try {
    return withLock(`inventory_${itemName}`, () => {
      ...
      sheet
        .getRange(rowNum, INVENTORY_COLS.RESERVED_QTY + 1)
        .setValue(newReservedQty);

      // Update available quantity
      const newAvailableQty = currentQty - newReservedQty;
      sheet
        .getRange(rowNum, INVENTORY_COLS.AVAILABLE_QTY + 1)
        .setValue(newAvailableQty);
      ...
      logAudit(
        ...
        reservedQty,
        newReservedQty,
        `Reserved for project: ${projectId}`,
      );
  ...
  try {
    return withLock(`inventory_${itemName}`, () => {
      ...
      if (reservedQty < quantity) {
        throw new Error(
          `Cannot release ${quantity}. Only ${reservedQty} reserved.`,
        );
      }
      ...
      logAudit(
        ...
        reservedQty,
        newReservedQty,
        `Released from project: ${projectId}`,
      );
 ...
 * @returns {object} - Result
 */
function sendApprovalResultNotification(
  submitterEmail,
  item,
  status,
  reason = '',
  ...
  try {
    const itemType = item.type === 'EXPENSE' ? 'Expense' : 'Income';
    const isApproved =
      status === STATUS.APPROVED || status === STATUS.PARTIALLY_APPROVED;
    ...
    console.error(
      `Error sending approval result notification: ${error.message}`,
    );
    return {
      success: false,
      error: error.message,
...
function sendLowStockAlert(lowStockItems) {
  try {
    ...
    const recipients = getRecipientsByRole([
      ROLES.FINANCE_MANAGER,
      ROLES.CEO,
      ROLES.OWNER,
    ]);
...
function scheduledDailyDigest() {
  try {
    const approvers = getRecipientsByRole([
      ROLES.CEO,
      ROLES.OWNER,
      ROLES.FINANCE_MANAGER,
      ROLES.MANAGER,
...
function getProjectsEnhanced(filters = {}) {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      project.formattedPendingRecovery = formatCurrency(
        project.pendingRecovery,
      );
      ...
      project.agingBucket =
        project.pendingRecovery > 0
          ? getAgingBucket(new Date(project.startDate))
          : null;
...
function getProjectExpenses(projectId) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (
      data[i][EXPENSES_COLS.PROJECT] === projectId &&
      data[i][EXPENSES_COLS.STATUS] === STATUS.APPROVED
    ) {
      const amount = parseFloat(data[i][EXPENSES_COLS.AMOUNT]) || 0;
      total += amount;
      expenses.push({
...
function getProjectIncome(projectId) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (
      data[i][INCOME_COLS.PROJECT] === projectId &&
      data[i][INCOME_COLS.STATUS] === STATUS.APPROVED
    ) {
      const amount = parseFloat(data[i][INCOME_COLS.AMOUNT]) || 0;
      total += amount;
      income.push({
  ...
  try {
    ...
    return withLock(`project_${projectId}`, () => {
      ...
      if (updates.endDate !== undefined) {
        updateMap[PROJECTS_COLS.END_DATE] = updates.endDate
      ...
      if (updates.contractValue !== undefined) {
        updateMap[PROJECTS_COLS.CONTRACT_VALUE] = parseFloat(
...
function recalculateProjectFinancials(projectId) {
  try {
    ...
    const marginPercent =
      receivedAmount > 0 ? (grossMargin / receivedAmount) * 100 : 0;

    // Update project sheet
    projectSheet
      .getRange(rowNum, PROJECTS_COLS.INVOICED_AMOUNT + 1)
      .setValue(invoicedAmount);
    projectSheet
      .getRange(rowNum, PROJECTS_COLS.RECEIVED_AMOUNT + 1)
      .setValue(receivedAmount);
    projectSheet
      .getRange(rowNum, PROJECTS_COLS.PENDING_RECOVERY + 1)
      .setValue(pendingRecovery);
    projectSheet
      .getRange(rowNum, PROJECTS_COLS.COST_TO_DATE + 1)
      .setValue(costToDate);
    projectSheet
      .getRange(rowNum, PROJECTS_COLS.GROSS_MARGIN + 1)
      .setValue(grossMargin);
    projectSheet
      .getRange(rowNum, PROJECTS_COLS.MARGIN_PERCENT + 1)
...
function updateInvoiceStatus(invoiceNo, status, paymentDate = null) {
  try {
    ...
    if (status === STATUS.PAID && paymentDate) {
      sheet
        .getRange(rowNum, INVOICES_COLS.PAYMENT_DATE + 1)
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      summary.totalContractValue +=
        parseFloat(data[i][PROJECTS_COLS.CONTRACT_VALUE]) || 0;
      summary.totalPendingRecovery +=
        parseFloat(data[i][PROJECTS_COLS.PENDING_RECOVERY]) || 0;
      summary.totalCost += parseFloat(data[i][PROJECTS_COLS.COST_TO_DATE]) || 0;
      summary.totalMargin +=
        parseFloat(data[i][PROJECTS_COLS.GROSS_MARGIN]) || 0;
    }

    summary.avgMarginPercent =
      summary.totalContractValue > 0
        ? (summary.totalMargin / summary.totalContractValue) * 100
        : 0;
